...
# ── DETECTA WINDOW MANAGER ────────────────────────────────
detect_wm() {
    if pgrep -x "openbox" > /dev/null; then echo "openbox"
    elif pgrep -x "xfwm4" > /dev/null; then echo "xfwm4"
    else echo "none"; fi
}
...
# 3. VERIFICAR WINDOW MANAGER (openbox ou xfwm4)
WM=$(detect_wm)
if [ "$WM" = "none" ]; then
    log "ALERTA: Nenhum Window Manager ativo. Tentando openbox..."
    if command -v openbox &>/dev/null; then
        DISPLAY=:0 openbox --replace &
        sleep 2
        log "openbox reiniciado."
    elif command -v xfwm4 &>/dev/null; then
        log "openbox nao encontrado. Tentando xfwm4..."
        ...
        sleep 2
        log "xfwm4 reiniciado."
    else
        log "ERRO: Nenhum WM disponivel (openbox/xfwm4)."
    fi
else
    log "WM OK: ${WM} rodando."
fi

# 4. VERIFICAR CHROMIUM
KIOSK_URL="https://ufrb.edu.br/portal/"
if ! pgrep -f "chromium.*kiosk" > /dev/null; then
    ...
    DISPLAY=:0 chromium --kiosk --start-maximized --noerrdialogs \
             --disable-infobars --incognito "${KIOSK_URL}" &
